...
LINMATH_H_FUNC void vec2_set(vec2 r, float x, float y) 
{
    r[0] = x; r[1] = y;
}
...
// world to screen
LINMATH_H_FUNC void vec4_project(vec4 r, mat4x4 VP, vec4 p, float w, float h) {
    mat4x4_mul_vec4(r, VP, p); 
...
LINMATH_H_FUNC void vec4_unproject(vec4 r, mat4x4 VP, vec4 p, float w, float h) {
   p[0] = p[0]/w*2.0 - 1.0;
   p[1] = 1.0 -	p[1]/h*2.0;
   mat4x4 VP_i;
   mat4x4_invert(VP_i, VP);
   mat4x4_mul_vec4(r, VP_i, p);
   r[0]/=r[3];
   r[1]/=r[3];
   r[2]/=r[3];
   r[3]/=r[3];
